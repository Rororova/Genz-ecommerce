...
const Blog = () => {
  ...
  const fetchArticles = async (pageNum = 1, append = false) => {
    try {
      ...
      // Fetch featured article separately on first page
      if (pageNum === 1) {
        const featuredResponse = await fetch('/api/articles?featured=true&limit=1')
        const featuredData = await featuredResponse.json()
        if (featuredData.success && featuredData.data.length > 0) {
          setFeaturedArticle(featuredData.data[0])
        }
      }

      // Fetch regular articles (exclude featured on first page)
      if (pageNum === 1) {
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2">
                    <div className="h-64 md:h-auto border-b-2 md:border-b-0 md:border-r-2 border-black relative overflow-hidden">
                      ...
                      />
                      <div className={`absolute top-4 left-4 ${featuredArticle.category_bg_color} border-2 border-black px-3 py-1 text-xs font-bold z-20`}>
                        FEATURED
                      </div>
                    </div>
                    ...
                    {articles.map((article) => (
                      ...
                      >
                        <div className="h-48 border-b-2 border-black overflow-hidden relative">
                          ...
                          />
                          <div className={`absolute bottom-2 right-2 ${article.category_bg_color} border border-black px-2 py-1 text-[10px] font-bold uppercase`}>
                            {article.category}
                          </div>
                        </div>
                        <div className="p-6 flex flex-col flex-grow">
                          <h3 className="font-syne text-2xl font-bold leading-tight mb-3">{article.title}</h3>
                          ...
                          <div className="mt-auto flex justify-between items-center pt-4 border-t-2 border-black/10">
                            <span className="text-xs font-bold text-gray-500">BY {article.author}</span>
                            <iconify-icon
